...
export function DeductionsList() {
    ...
    if (deductions.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-zinc-500 text-sm">No late deductions found</p>
            </div>
        );
    }

    // Group by employee
    const groupedByEmployee = deductions.reduce((acc, record) => {
    ...
    return (
        <div className="space-y-3">
            {Object.entries(groupedByEmployee).map(([email, records]) => {
                                        ...
                                        return (
                                            <>
                                                <div className="bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-lg p-3 text-center">
                                                    ...
                                                    <p className="text-lg font-bold text-red-700">₹ {(thisMonthAmount / 100).toFixed(2)}</p>
                                                </div>
                                                <div className="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-lg p-3 text-center">
                                                    ...
                                                    <p className="text-lg font-bold text-orange-700">₹ {(lastMonthAmount / 100).toFixed(2)}</p>
                                                </div>
                                                <div className="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-lg p-3 text-center">
                                                    ...
                                                    <p className="text-lg font-bold text-amber-700">₹ {(lastSixMonthsAmount / 100).toFixed(2)}</p>
                                                </div>
                                            </>
                                        );
                                    })()}
                                        ...
                                        <div key={record.id} className="flex items-center justify-between bg-red-50 p-3 rounded-lg">
                                            <div className="flex items-center gap-3 flex-1">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="font-medium text-sm text-zinc-900 capitalize">{record.reason}</p>
