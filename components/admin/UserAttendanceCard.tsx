...
export function UserAttendanceCard({ user, current, onChange }: UserAttendanceCardProps) {
    ...
    }, [current.status, current.lateMinutes, current.extraHours]);

    async function handleStatusChange(s: 'present' | 'leave' | 'half_day' | 'off' | 'unmarked') {
        setStatus(s);
        const newLate = s === 'present' ? late : 0;
        const newExtra = s === 'present' ? extraHours : 0;
        setLoading(true);
        await onChange(user.id, { status: s, lateMinutes: newLate, extraHours: newExtra });
        setLoading(false);
    }
    ...
    async function handleLateChange(l: number) {
        ...
        if (status === 'present') {
            setLoading(true);
            await onChange(user.id, { status, lateMinutes: l, extraHours });
            setLoading(false);
        }
    }
    ...
    async function handleExtraHoursChange(h: number) {
        ...
        if (status === 'present') {
            setLoading(true);
            await onChange(user.id, { status, lateMinutes: late, extraHours: h });
            setLoading(false);
        }
    }
    ...
    return (
        <div className="flex-shrink-0 w-[280px] h-[360px] bg-white rounded-3xl border border-zinc-200 shadow-sm p-4 flex flex-col justify-between hover:shadow-md transition-shadow">
            ...
            <div className="space-y-3">
                ...
                <div className={cn("transition-all duration-300 overflow-hidden", status === 'present' ? "max-h-40 opacity-100" : "max-h-0 opacity-0")}>
                    <p className="text-[9px] font-semibold text-zinc-400 uppercase mb-1">Late Arrival?</p>
                    <select
                        value={late}
