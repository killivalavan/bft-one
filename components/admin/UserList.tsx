...
import {
  ...
  Calendar, ChevronDown, Mail, ShieldAlert
} from "lucide-react";
...
function UserRow({ user, onRemoveUser, onUpdatePass, onToggleStockManager, onUpdateFullProfile, onDownloadPayslip }: {
  ...
  async function handleSave() {
    ...
    if (updates.per_day_salary_cents) {
      ...
      await supabaseClient.from('salary_entries')
      ...
      <Card className="border-indigo-100 bg-indigo-50/30">
        <CardContent className="p-4 sm:p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-1.5 text-xs text-zinc-600 font-medium cursor-pointer">
                <input
                  type="checkbox"
                  checked={sendInvite}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 items-center">
            ...
            <div className="sm:col-span-3">
              <Button
                ...
                className="bg-indigo-600 hover:bg-indigo-700 text-white w-full h-10 font-semibold"
              >
                <UserPlus size={16} className="mr-2" />
                <span>{sendInvite ? "Send Invite" : "Create User"}</span>
              </Button>
            </div>
          </div>
          ...
          <p className="text-[11px] text-zinc-500">
            {sendInvite
              ...
              : "Staff passwords can be flexible (e.g. DOB 12092026 or PIN), and they can update it anytime in their profile."}
          </p>
        </CardContent>
      </Card>
